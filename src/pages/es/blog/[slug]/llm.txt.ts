...
function createMetadataLines(post: BlogEntry): string[] {
  const lines = [
    ...
    `Publicado: ${formatDate(post.data.date)}`,
  ];

  if (post.data.updatedDate) {
    lines.push(`Actualizado: ${formatDate(post.data.updatedDate)}`);
  }

  if (post.data.author) {
    lines.push(`Autor: ${post.data.author}`);
  }

  if (post.data.keywords.length) {
    lines.push(`Palabras clave: ${post.data.keywords.join(', ')}`);
  }
...
export const GET: APIRoute = async ({ props, site, url }) => {
  ...
  const markdown = [
    `# ${post.data.title}`,
    '',
    `> Si eres una persona y prefieres leer este texto en la página web, visita [la versión web](${humanPageUrl}).`,
    '',
